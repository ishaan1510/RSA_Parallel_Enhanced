...
def brute_force_attack(public_key, ciphertext):
    ...
    for possible_d in range(2, n):
        possible_plain = []
        is_valid = True  # Assume the key is valid unless proven otherwise
        ...
        for char in ciphertext:
            ...
            if 32 <= decoded_char <= 126:  # Check for valid ASCII characters
                ...
                is_valid = False  # If an invalid character is found, mark the key as invalid
                break  # Stop checking further if invalid characters are found

        if is_valid:  # Only process the message if all characters are valid
            possible_message = ''.join(possible_plain)
            print(f"Trying private key 'd': {possible_d} -> Possible Message: {possible_message}")
            
            # Heuristic check: does the message contain common English words?
            if " " in possible_message or any(word in possible_message.lower() for word in ["hola", "amigos"]):
